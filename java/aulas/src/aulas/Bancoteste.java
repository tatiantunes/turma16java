	...
	public static void main(String[] args)
	{
		...
		decisaoFinal = leia.nextInt();
		 if (decisaoFinal == 1)
		 {
		  ...
		  {
			recomecar = false;
			System.out.println("Voce deseja emitir extrato na sua saida?(S/N)");
			decisaoExtrato = leia.next().charAt(0);
			...
			if (decisaoExtrato != 'N')
				...
				System.out.println("\t\t\t\t" + "DEPRESSÃO BANKING G16");
				System.out.println("***************************************************************************************************");
				System.out.println("Saida" + "\t\t\t" + "Saldo" + "\t\t\t" + "Limite Disponivel" + "\t\t\t" +  "Descricão");
				for(int x = 0 ; x <2 ;x ++) 
					{
					
					System.out.println((x+1) + "º" + "\t\t\t " +saldomuitos[x]+ " \t\t\t\t " + limiteDisponivelMuitos[x]+ "\t\t\t\t " + descricaodaTransacao[x] );
